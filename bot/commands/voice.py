...
def setup_voice(client: commands.Bot):
    ...
    async def vc_leave(ctx: commands.Context):
        ...
        if not is_authorized:
            ...
            await ctx.reply(
                f"only {joiner_name} (who ran `/vc join`) can disconnect the bot",
                ephemeral=True,
            )
            return
    ...
    @vc_group.command(
        name="bird", description="make the bot say bird in the voice channel"
    )
    ...
    @vc_group.command(
        name="fart", description="make the bot fart in the voice channel"
    )
    ...
    @client.hybrid_command(
        name="play",
        description="play a sound from a link, uploaded file, or attachment",
    )
    @app_commands.describe(
        link="the link (youtube/direct) to play", file="an uploaded audio file to play"
    )
    async def play_cmd(
        ctx: commands.Context, link: str = None, file: discord.Attachment = None
    ):
        if ctx.voice_client is None:
            await ctx.reply("i'm not in a voice channel. use `/vc join` first")
            return
        ...
        if attachment:
            ...
            if (
                "youtube.com" in link_clean
                or "youtu.be" in link_clean
                or "youtube" in link_clean
            ):
                try:
                    ...
                    if proc.returncode == 0:
                        ...
                        await ctx.reply(
                            f"failed to extract audio: {stderr.decode('utf-8')}"
                        )
                        return
                except Exception as e:
        ...
        if not source:
            await ctx.reply(
                "you must either upload an audio file or specify a link to play"
            )
            return
    ...
    @bad_apple_command := bad_group.command(
        name="apple", description="play bad apple audio in voice channel"
    )
    @app_commands.describe(
        max_volume="play at 100% volume for 1000 coins instead of 60% for 60 coins"
    )
    async def bad_apple(ctx: commands.Context, max_volume: bool = False):
        ...
        if balance < cost:
            coin_emoji = await asyncio.to_thread(db.get_config, "coin_emoji", "🪙")
            await ctx.reply(
                f"you don't have enough coins. this costs {cost} {coin_emoji} (your balance: {balance})"
            )
            return
        ...
        try:
            ...
            new_balance = await asyncio.to_thread(
                db.update_balance, ctx.author.id, -cost
            )
            coin_emoji = await asyncio.to_thread(db.get_config, "coin_emoji", "🪙")
            await ctx.reply(
                f"queued bad apple 🍎 ({'100%' if max_volume else '60%'} volume). deducted {cost} {coin_emoji} (balance: {new_balance})"
            )
        except Exception as e:
            await ctx.reply(f"error playing audio: {e}")
